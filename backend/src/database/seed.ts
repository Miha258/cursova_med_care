...
async function seed() {
  ...
  await prescriptionRepo.save([
    prescriptionRepo.create({ patientId: patients[0].id, doctorId: doctors[1].id, items: [{ name: 'Амлодипін 5мг',      dosage: '1 таб.',   frequency: '1 р/день', duration: '30 днів',  instruction: 'Вранці до їжі'   }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[0].id, doctorId: doctors[1].id, items: [{ name: 'Лозартан 50мг',      dosage: '1 таб.',   frequency: '1 р/день', duration: '30 днів',  instruction: 'Ввечері'          }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[1].id, doctorId: doctors[0].id, items: [{ name: 'Парацетамол 500мг',  dosage: '1-2 таб.', frequency: '3 р/день', duration: '5 днів',   instruction: 'При температурі'  }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[1].id, doctorId: doctors[0].id, items: [{ name: 'Омепразол 20мг',     dosage: '1 капс.',  frequency: '1 р/день', duration: '14 днів',  instruction: 'До їжі'           }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[2].id, doctorId: doctors[2].id, items: [{ name: 'Диклофенак 50мг',    dosage: '1 таб.',   frequency: '2 р/день', duration: '7 днів',   instruction: 'Після їжі'        }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[2].id, doctorId: doctors[2].id, items: [{ name: 'Пірацетам 400мг',    dosage: '2 капс.',  frequency: '3 р/день', duration: '30 днів',  instruction: ''                 }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[3].id, doctorId: doctors[0].id, items: [{ name: 'Левотироксин 50мкг', dosage: '1 таб.',   frequency: '1 р/день', duration: 'постійно', instruction: 'Натщесерце'       }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[4].id, doctorId: doctors[1].id, items: [{ name: 'Метформін 500мг',    dosage: '1 таб.',   frequency: '2 р/день', duration: 'постійно', instruction: 'Під час їжі'      }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[4].id, doctorId: doctors[1].id, items: [{ name: 'Аторвастатин 20мг',  dosage: '1 таб.',   frequency: '1 р/день', duration: 'постійно', instruction: 'Ввечері'          }], status: PrescriptionStatus.ACTIVE }),
    prescriptionRepo.create({ patientId: patients[5].id, doctorId: doctors[2].id, items: [{ name: 'Карбамазепін 200мг', dosage: '1 таб.',   frequency: '2 р/день', duration: '30 днів',  instruction: 'Під час їжі'      }], status: PrescriptionStatus.ACTIVE }),
  ]);
  console.log('✅ Prescriptions: 10');

  // ── LAB TESTS ──────────────────────────────────────────────────────────────
  await labTestRepo.save([
    labTestRepo.create({ patientId: patients[0].id, testName: 'Загальний аналіз крові',   result: 'Гемоглобін 138 г/л, Лейкоцити 6.2×10⁹/л', status: LabTestStatus.NORMAL,   notes: 'Норма'                       }),
    labTestRepo.create({ patientId: patients[0].id, testName: 'Ліпідний профіль',         result: 'ЗХС 6.1, ЛПНЩ 3.9, ЛПВЩ 1.1',             status: LabTestStatus.ABNORMAL, notes: 'Помірна гіперхолестеринемія' }),
    labTestRepo.create({ patientId: patients[1].id, testName: 'Загальний аналіз сечі',    result: 'Питома вага 1.018, Білок відсутній',         status: LabTestStatus.NORMAL,   notes: 'Без патології'               }),
    labTestRepo.create({ patientId: patients[1].id, testName: 'ТТГ',                      result: '',                                          status: LabTestStatus.PENDING,  notes: 'В очікуванні'                }),
    labTestRepo.create({ patientId: patients[2].id, testName: 'МРТ шийного відділу',      result: 'Протрузія С5-С6 2мм',                       status: LabTestStatus.ABNORMAL, notes: 'Направлення до нейрохірурга' }),
    labTestRepo.create({ patientId: patients[2].id, testName: 'Загальний аналіз крові',   result: 'Гемоглобін 125 г/л, ШОЕ 18',               status: LabTestStatus.ABNORMAL, notes: 'Легка анемія'                }),
    labTestRepo.create({ patientId: patients[3].id, testName: 'ЕКГ',                      result: 'Синусовий ритм, ЧСС 72/хв',                 status: LabTestStatus.NORMAL,   notes: 'Норма'                       }),
    labTestRepo.create({ patientId: patients[4].id, testName: 'HbA1c',                    result: '7.2%',                                      status: LabTestStatus.ABNORMAL, notes: 'Діабет під контролем'        }),
    labTestRepo.create({ patientId: patients[4].id, testName: 'Глюкоза крові',            result: '8.1 ммоль/л',                               status: LabTestStatus.ABNORMAL, notes: 'Підвищена'                   }),
    labTestRepo.create({ patientId: patients[5].id, testName: 'МРТ головного мозку',      result: '',                                          status: LabTestStatus.PENDING,  notes: 'Терміново'                   }),
    labTestRepo.create({ patientId: patients[6].id, testName: 'УЗД черевної порожнини',   result: 'Жовчний міхур: конкременти 5-8мм',          status: LabTestStatus.ABNORMAL, notes: 'Консультація хірурга'        }),
  ]);
